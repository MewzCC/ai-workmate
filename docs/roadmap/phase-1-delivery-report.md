...
- 后端：Java 17 + Maven 测试通过，47 tests，0 failures，0 errors，1 skipped。
- 数据库：本机 PostgreSQL 16 空库连续执行 `init.sql` 两次通过。
- OA：Vitest 2 个测试文件、5 项测试通过；lint 和生产 build 通过。
