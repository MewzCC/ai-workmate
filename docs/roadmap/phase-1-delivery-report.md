...
- 草稿允许在审批链路暂未配置时先保存；用户也可以从可搜索候选列表中选择本次审批人，候选人限定为申请人本部门或祖先部门内、同租户、启用且实时拥有 `approval:act` 的非本人用户。
- 提交时重新验证所选审批人；所选人员缺失或因组织、权限变化而失效时，服务端按“直属审批人优先、部门默认审批人兜底”重新解析，仍无有效审批人才拒绝提交。
