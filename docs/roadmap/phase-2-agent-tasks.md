...
| 编号 | 任务 | 内容 | 产出 | 依赖 |
| --- | --- | --- | --- | --- |
| A1 | agent_tool 表与种子数据 | `init.sql` 新增 `agent_tool`；JSONB schema、平台/租户两组部分唯一索引、风险/权限 CHECK；租户覆盖只能收窄能力；幂等插入 4 个 L0 工具 | 可重复执行的 init.sql 迁移段 | P2/P3 |
...
| 编号 | 任务 | 内容 | 产出 | 依赖 |
| --- | --- | --- | --- | --- |
| B1 | 任务表结构 | `init.sql` 新增 agent_task、agent_task_step、agent_task_event；外键、CHECK、taskNo、幂等域、taskId+seq 唯一约束和列表/领取/过期索引 | 可重复执行的 init.sql 迁移段 | P1/P2/P3 |
...
| B8 | 步骤执行器 | 重新校验权限、资源、工具版本与 schema；按序执行；记录 attempt、结果摘要和错误；部分成功进入人工处理提示 | StepExecutor + 测试 | A5、B7 |
...
| 编号 | 任务 | 内容 | 产出 | 依赖 |
| --- | --- | --- | --- | --- |
| C1 | DTO 与错误码 | plan/execute/列表/详情/取消/SSE DTO；Bean Validation 使用 i18n key；新增权限变化、工具版本变化、确认过期、幂等冲突等错误码 | DTO + MessageSource zh/en | P1 |
...
| C4 | Policy Guard | 校验工具、版本、schema、实时权限、数据范围、风险、步骤/调用预算和永久禁止清单；Phase 2B 最多一个写步骤；非法计划直接拒绝 | PolicyGuard + 测试 | C3 |
...
| D5 | 我的 AI 任务页 | Ant Design Table 列表、状态筛选、步骤详情、终态结果、失败原因和取消；服务端动态路由、组件注册、OaIcon | AiTasksPage + init.sql 路由 | D1/C7 |
...
| 编号 | 任务 | 内容 | 产出 | 依赖 |
| --- | --- | --- | --- | --- |
| G1 | 数据库验证 | init.sql 空库和重复执行；验证约束、索引、种子数据与租户覆盖 | 验证记录 | A1/B1/D5 |
| G2 | 安全与恢复演练 | 普通员工 L0、跨租户、权限回收、并发 execute、进程重启、SSE 重连、直接/间接注入、永久禁止能力、限流、Kill Switch、审计异常 fail closed | 人工逐项演练记录 | A~D |
...
| E2 | leave.createDraft | L1；调用 LeaveWorkflowService 创建本人草稿；使用领域幂等键避免 Worker 重试产生重复草稿；写审计 | handler + 测试 | E1/B8 |
| E3 | leave.submit | L2；仅提交执行前已存在的本人有效草稿；不得与 createDraft 同计划；要求 planVersion/planHash/一次性凭证；使用领域状态条件更新防重复 | handler + 测试 | E2 |
...
| F2 | 审计边界 | 成功写入、安全拒绝、确认重放、非法 schema、越权和幂等冲突写脱敏审计 | 审计实现 + 查询验证 | A4/B5/C4 |
| F3 | 数据留存与清理 | input/pageContext/args/result/event 的留存期限、脱敏、清理和人工查询边界 | 配置、Cleaner 与说明 | P3/B10 |
| F4 | 架构文档 | `docs/architecture/agent-task-engine.md`：状态机、幂等、确认、Worker 租约、SSE、权限和时序图 | 架构文档 | A~C |
...
排期建议：一名人工负责人持续使用 Codex/AI 协作，安排 4~6 个自然周并将 Phase 2A、Phase 2B 分两次发布。AI 可并行生成样板代码和测试骨架，但人工安全评审、失败路径测试和发布门不得折算或跳过。
...
P1/P2/P3
  ├─ A1 → A3 → A5 → A6 ─┐
  └─ B1 → B3 → B6 → B7 ─┼→ C3 → C4 → C5/C7 → D1 → D3/D5 → G1~G4
                          ├→ B9 → C9 → D4 ────────────────┤
                          └→ B12 → G2 ────────────────────┘
