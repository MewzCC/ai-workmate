...
| E4 | leave.apply 原子申请 | L2；一句话只产生一个写步骤；通过 Tool Gateway 后调用领域 Service，在同一事务内创建本人申请、提交、启动审批流程并写审计；禁止调用其他 Handler 或二次规划 | handler + 事务/网关测试 | E3 |
| E5 | 写操作联合流程 | 计划、确认、原子入队、Worker 执行、SSE、任务详情、审计中心完整可见 | E2E 联调用例 | D3/D5/E4 |
| E6 | 写工具安全测试 | 未确认、凭证重放、计划变化、权限回收、他人草稿、跨租户参数、同键异请求、原子回滚、Worker 重启与不可重试失败 | 安全与恢复测试 | E4/E5 |
...
G4 通过 → E1 → E2 → E3 → E4 → E5/E6 → Phase 2B 验收
```
