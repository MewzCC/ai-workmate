...
## 8. 后续原子工具扩展（2026-09-08，待人工验收）

为支持“一句话完成本人请假申请”，在既有发布门之上新增 `leave.apply` L2 原子领域工具。它仍然只占一个 `SINGLE_WRITE` 步骤，要求二次确认与一次性凭证，并只能经进程内 `ToolGateway` 分派。Handler 不组合 `leave.createDraft` 与 `leave.submit`，而是调用领域 Service，在同一数据库事务中完成本人申请创建、提交、审批流程启动和业务审计；任一步失败则整体回滚。

该扩展不改变永久禁止清单，不接受 userId、tenantId、权限、URL、SQL 或文件路径等越权参数，也不允许跨租户、跨用户、跨领域调用、二次规划或绕过网关。本文前述 2026-08-29 结论保留为当时交付记录；新增工具必须重新通过自动化、Flyway 实库和人工发布门后方可视为验收完成。
