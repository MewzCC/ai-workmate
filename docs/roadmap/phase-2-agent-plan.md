...
同时完成 Tool Registry、持久任务状态机、异步执行、确认机制、审计与失败分类，为后续更多写操作工具、UI Command 协议和多 Agent 提供可信执行底座。
...
- 采用服务端 Tool Registry + Policy Guard + 确定性执行引擎，不直接依赖模型 provider 的原生 function calling 回调执行工具。
...
- 新增“我的 AI 任务”页面：Ant Design Table 列表、步骤详情、终态结果、失败原因和可取消操作；入口作为服务端动态路由写入 `init.sql`。
...
- 失败分类至少包括：AUTH_FAILED、PERMISSION_DENIED、RESOURCE_SCOPE_DENIED、RETRIEVAL_EMPTY、MODEL_TIMEOUT、SCHEMA_INVALID、TOOL_VERSION_CHANGED、TOOL_TIMEOUT、TOOL_FAILED、CONFIRMATION_EXPIRED、IDEMPOTENCY_CONFLICT、TASK_TIMEOUT。
- 若本期实现运行预算上限，再启用 `BUDGET_EXCEEDED`；不实现预算控制时不应返回该错误码。
...
- 四个 L0 工具。
- Tool Registry、Policy Guard、持久状态机、数据库队列 Worker、SSE、任务查询和取消。
...
| M2 持久任务引擎 | 状态机、Worker 租约、幂等、任务查询/取消、SSE 续传 | 4~6 天 |
| M3 Planner、Policy Guard 与 API | 结构化计划、上下文过滤、策略复核、任务 API 与攻击用例 | 3~4 天 |
| M4 Phase 2A 前端闭环 | AI Drawer、任务记录页、真实错误态和断线恢复 | 2~3 天 |
| M5 Phase 2A 安全发布门 | 越权、注入、并发、重启、Kill Switch 和人工逐项验收 | 2~3 天，不得压缩 |
| M6 Phase 2B 写操作试点 | 两个独立请假写工具、确认凭证、领域幂等和安全验收 | 2~3 天 |
| M7 观测、治理与文档 | 观测、审计、数据清理、规则和架构文档同步 | 1~2 天，贯穿执行 |

总量按 17~26 个 Vibe Coding 有效工作日估算，其中代码生成和测试骨架可以并行，但数据库约束、鉴权、确认、幂等、Worker 领取 SQL 与安全发布门必须由人工逐项评审。建议安排 4~6 个自然周，Phase 2A 与 Phase 2B 分两次发布；不得因为 AI 已生成代码而跳过失败路径测试或安全门。
...
- `init.sql` 空库执行和重复执行通过，约束、索引与种子数据符合设计。
