...
- 数据库只维护 `backend/src/main/resources/db/init.sql`，由部署流程统一执行；脚本必须幂等，并提供回滚或前向修复说明。
...
- IAM/Workflow 的 `init.sql` 结构变更与数据字典。
- 权限矩阵和测试租户初始化方案。
- 待办、请假、单笔审批前后端完整链路。
- 业务审计查询接口和最小审计页面。
...
- Knowledge API、`init.sql` 结构变更、入库 worker 和对象存储适配。
