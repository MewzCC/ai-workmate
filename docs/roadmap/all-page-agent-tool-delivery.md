...
核对日期：2026-09-17。代码依据为 `PageCapabilityCatalog`、`ToolCode`、代码工具契约和对应领域 Port。目录登记不等同于运行启用、接口完整或浏览器验收通过。已登记写工具的全局与租户写开关仍默认关闭，尚未通过生产人工发布门及真实 LLM 端到端验收。
...
P7 资产领用原子工具切片：新增 `asset.claim` 封闭契约，只接收资产 ID、目标员工 ID、预期版本和可选原因；租户、操作人、权限及员工部门均来自可信服务端上下文。Handler 通过既有类型化 `AssetToolPort` 调用资产领域 Service，领域再次校验实时 `asset:claim` 权限、资产与员工租户、员工有效状态、部门、资产空闲状态和乐观锁版本。稳定操作键、源版本和结果版本保存到资产操作记录，同一操作键由数据库唯一索引防重；资产更新、操作历史和业务审计处于同一事务。工具为 L1、租户范围、显式确认、业务幂等和单写步骤，全局与租户写开关继续默认关闭。真实 PostgreSQL 已覆盖并发重放只产生一条操作和审计、命令冲突、审计失败回滚、跨租户不可见及权限回收；空库、旧库升级、76 个迁移 validate 与二次启动零迁移通过。资产归还和维修登记仍须按独立里程碑交付。

P7 资产归还原子工具切片：新增 `asset.return` 封闭契约，只接收资产 ID、预期版本和可选原因，复用 `AssetToolPort`、本地 Adapter、稳定操作键及显式结果核验协议。领域重新解析实时 `asset:return` 权限和租户，只有使用中的资产可按乐观锁归还；成功后清除保管人但保留所属部门，资产更新、操作历史和业务审计同事务。并发重放在状态已改变时先核验同一操作键收据，参数不一致或非同一操作继续失败关闭。工具为 L1、租户范围、显式确认、业务幂等和单写步骤，写开关保持默认关闭。真实 PostgreSQL 已覆盖并发只落一条归还操作和审计、保管人清除、结果核验及审计失败回滚；空库、旧库升级、77 个迁移 validate 与二次启动零迁移通过。维修登记仍须独立交付。

P7 资产维修登记原子工具切片：新增 `asset.repair.start` 封闭契约，只接收资产 ID、预期版本和必填故障原因；可信租户、用户与权限不进入模型参数。资产领用、归还和维修登记共用领域内的版本绑定状态迁移模板，集中处理幂等回放、并发竞态后的收据核验、乐观锁、操作记录和事务审计，各工具仅声明自己的权限、来源状态、目标状态及专属校验。领域重新解析实时 `asset:repair` 权限，仅允许把单件 `IDLE` 资产置为 `REPAIRING`，不包含维修完成、报废或批量操作。工具继续为 L1、租户范围、显式确认、业务幂等和单写步骤，全局与租户写开关默认关闭；Port/Adapter 边界不依赖 Spring MVC，后续拆分 Spring Cloud 时可替换为受认证的远程领域适配器而不改 ToolGateway 契约。真实 PostgreSQL 已覆盖并发重放只产生一条维修操作与审计、结果核验和审计失败整笔回滚；空库、旧库升级、78 个迁移 validate 与二次启动零迁移通过。
...
R6 全页面能力一致性门禁切片：后端启动门禁现在逐项比较 `OaPage` 与 Agent 页面能力目录，任何页面缺失、额外页面或组件错配都会阻止应用启动；前端契约测试直接读取同仓后端清单，校验 41 个启用组件、渲染注册表和唯一性，避免跨语言手工清单静默漂移。真实 PostgreSQL 迁移门禁查询全部启用 PAGE 的 `route_key + component_key`，同时覆盖空库、模拟旧库升级、Flyway validate 和重启零迁移。本切片不新增运行接口、工具、迁移或写能力。OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过；后端 828 项零失败、9 项既有环境测试跳过；真实 PostgreSQL 16 空库及旧库升级、75 个迁移 validate、重启零迁移和 41 条页面路由逐项一致均通过。
