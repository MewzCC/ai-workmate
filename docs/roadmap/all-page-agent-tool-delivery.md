...
| employee-change | HR_CHANGE_QUERY | HR_CHANGE_APPLY；租户内单条员工变动申请，写开关默认关闭；只提交审批，不直接生效或修改员工档案 |
...
| seal-usage | SEAL_QUERY | SEAL_APPLY、SEAL_REGISTER_USE；本人单条用印申请与已审批实际用印登记，写开关默认关闭；登记需二次确认，不包含归还或文件上传 |
...
P8 访客申请原子工具切片：新增 `visitor.apply` 封闭契约，只接收访客姓名、公司、电话、来访事由、被访人、预计到离时间、车牌和人数；申请人、租户及操作人只取 ToolGateway 可信上下文。Handler 通过类型化 `VisitorToolPort` 调用既有访客领域 Service，普通页面与 Agent 共用申请校验、审批实例、首待办、动作流水和通知创建逻辑；领域重新校验实时 `visitor:create` 权限、申请人和被访人有效性、同租户关系、到离时间及非本人审批人。稳定操作键由申请人范围唯一索引约束，相同命令并发重放只创建一条申请及一套审批数据，参数冲突失败关闭；写入与事务审计同事务，审计失败整笔回滚。工具为 L1、本人范围、显式确认、业务幂等和单写步骤，全局与租户写开关继续默认关闭。真实 PostgreSQL 已覆盖并发幂等、结果核验、跨租户不可见、权限回收及审计回滚；空库、旧库升级、79 个迁移 validate 与重复迁移零变更通过。开发库已升级到 V202609171650，二次启动无待执行迁移；OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过，后端 853 项零失败、9 项既有环境测试跳过。该工具只创建 `PENDING` 申请，不执行签到、到访、离场或审批决定；这些能力仍须独立交付和评审。
...
P8 用印申请原子工具切片：新增 `seal.apply` 封闭契约，只接收印章类型、文件标题、用印事由和份数；申请人、租户及操作人始终取自 ToolGateway 可信上下文。Handler 通过类型化 `SealToolPort` 调用既有行政资产领域 Service，普通页面与 Agent 共用申请校验、审批流程解析、实例、首待办、动作流水及通知创建逻辑，不复制审批状态机。领域重新校验实时 `seal:create` 权限、有效申请人、同租户审批人和表单约束；稳定操作键由申请人范围唯一索引约束，相同命令并发重放只创建一条申请、一套审批数据和一条事务审计，参数冲突、权限回收、跨租户及审计异常均失败关闭。类型化只读核验按可信身份和稳定操作键返回当前申请状态，不调用写入口。工具为 L1、本人范围、显式确认、业务幂等和单写步骤，全局及租户写开关继续默认关闭；它只创建 `PENDING` 用印申请，不执行审批决定、实际用印、文件上传或归还登记。真实 PostgreSQL 空库、旧库升级、83 个迁移 validate、并发幂等、结果核验、审计回滚和重复迁移零变更通过；开发库已升级至 V202609172250，二次启动无待执行迁移。OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过，后端 871 项零失败、9 项既有环境测试跳过。

P8 实际用印登记原子工具切片：新增 `seal.registerUse` 封闭契约，只接收已审批用印申请 ID、乐观锁版本、实际份数和可选备注；租户、申请人及操作人始终取自 ToolGateway 可信上下文。Handler 继续通过类型化 `SealToolPort` 与本地 Adapter 调用行政资产领域 Service，普通页面与 Agent 共用 `APPROVED → USED` 状态迁移、份数上限、版本更新和事务审计逻辑，不复制用印状态机。领域重新校验实时 `seal:register` 权限、本人资源归属、审批状态、实际份数和版本；`seal:register:any` 不扩大 Agent 的本人数据范围。稳定操作键由操作回执表唯一约束，相同命令并发重放只完成一次状态变化和一次审计，参数冲突、权限回收、跨租户、管理员代办及审计异常均失败关闭；类型化只读核验可在未知结果时查询已落库回执，但工具的 `NEVER` 重试策略不允许 Planner 自动重复写入。工具为 L2、本人范围、二次确认、业务幂等和单写步骤，全局及租户写开关继续默认关闭；它不执行审批决定、物理盖章、文件上传或归还登记。真实 PostgreSQL 空库、旧库升级、84 个迁移 validate、并发单写、结果核验、审计回滚和重复迁移零变更通过；开发库已升级至 V202609172330，二次启动无待执行迁移。OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过，后端 876 项零失败、9 项既有环境测试跳过。

P9 员工变动申请原子工具切片：新增 `hr.change.apply` 封闭契约，只接收租户内员工、变动类型、生效日期、目标组织信息、审核人和原因；申请人、租户及操作人始终取自 ToolGateway 可信上下文。Handler 通过类型化 `EmployeeChangeToolPort` 与本地 Adapter 调用现有员工变动领域 Service，普通页面与 Agent 共用员工、部门、岗位、直属审批人、重复待审申请和表单约束，不复制入转调离状态机。领域重新校验实时 `hr:manage` 权限、员工与审核人租户归属、审核人与申请人分离、未来或当日生效日期及目标组织有效性；稳定操作键由申请人范围唯一索引约束，相同命令并发重放只创建一条 `PENDING` 申请和一条事务审计，参数冲突、权限回收、跨租户及审计异常均失败关闭。类型化只读核验按可信身份和稳定操作键返回已提交回执，不调用写入口；历史申请即使生效日期已过去仍可核验，但新提交继续拒绝过去日期。工具为 L1、租户范围、显式确认、业务幂等和单写步骤，全局及租户写开关继续默认关闭；它不执行批准、驳回、生效、员工档案修改、角色或权限变更。真实 PostgreSQL 空库、旧库升级、85 个迁移 validate、并发幂等、结果核验、审计回滚和重复迁移零变更通过；开发库已升级至 V202609181120，二次启动无待执行迁移。OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过，后端 884 项零失败、9 项既有环境测试跳过。

P10 费用报销草稿原子工具切片：新增 `expense.createDraft` 封闭契约，只接受金额、费用类别、发生日期、发票号和事由，租户、申请人及操作人始终来自 ToolGateway 可信上下文。费用领域门面固定使用 `expense-application` 表单，通过类型化 `FinanceToolPort` 复用通用审批草稿事务、表单校验和审计，不复制审批状态机，也不允许模型选择任意表单或流程。领域重新校验实时 `route:approval-start` 与 `approval:create` 权限；稳定操作键由申请人范围唯一索引约束，相同命令并发重放只创建一条 `DRAFT` 申请和一条事务审计，参数冲突、跨租户、权限回收和审计异常均失败关闭。类型化只读核验按可信身份和稳定操作键查询已落库结果，不调用写入口；通用审批 Record 查询同时修正为按构造参数顺序映射，列表以空值占位快照字段，避免状态、版本错位及无意义加载大快照。工具为 L1、本人范围、显式确认、业务幂等和单写步骤，全局及租户写开关继续默认关闭；它不提交审批、不付款、不签署、不上传附件或对外发送。真实 PostgreSQL 空库、旧库升级、86 个迁移 validate、并发幂等、结果核验、审计回滚和重复迁移零变更通过；开发库已升级至 V202609181200，二次启动无待执行迁移。OA lint 无错误（3 条既有警告）、93 项测试和生产构建通过，后端 890 项零失败、9 项既有环境测试跳过。
